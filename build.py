...
def data_rows(case):
    ...
    for role in ROLES:
        if not is_role_enabled(case, role):
            continue
        rd = case["pbmc"][role]
            ...
            if key == "transaction":
                for flow in flows:
                    if flow.get("from") == role:
                        other, arrow, direction = flow.get("to",""), "→", "outgoing"
                    elif flow.get("to") == role:
                        other, arrow, direction = flow.get("from",""), "←", "incoming"
                    else:
                        continue
                    rows.append({
                        "Perspective": ROLE_NAMES[role],
                        "Row Type": "transaction_flow",
                        "Field": "↳",
                        "Value": f"{arrow} {ROLE_NAMES.get(other, other.title())} · {flow.get('value','')}",
                        "Direction": direction,
                        "Counterparty": ROLE_NAMES.get(other, other.title()),
                        "Flow": flow.get("value",""),
                        "Explanation": flow.get("explanation",""),
                    })
    cvu = case["pbmc"]["core_value_unit"]
    rows.append({
        "Perspective":"Core Value Unit","Row Type":"field","Field":"Core Value Unit",
        "Value":cvu.get("value",""),"Direction":"","Counterparty":"","Flow":"",
...
def write_csv(case_dir, case):
    rows = data_rows(case)
    fields = ["Perspective","Row Type","Field","Value","Direction","Counterparty","Flow","Explanation"]
    with (case_dir/"pbmc-data.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w = csv.DictWriter(f, fieldnames=fields)
        w.writeheader(); w.writerows(rows)

def flow_rows(case, role):
    out = []
    for f in active_flows(case):
        if f.get("from") == role:
            other, arrow = f.get("to",""), "→"
        elif f.get("to") == role:
            other, arrow = f.get("from",""), "←"
        else:
            continue
        out.append(f"""<tr class="flow-subrow">
<td>↳</td>
<td><span class="flow-direction"><span class="flow-arrow {ROLE_FLOW_CSS[role]}">{arrow}</span>{e(ROLE_NAMES.get(other,other.title()))} · {e(f.get("value"))}</span></td>
<td>{e(f.get("explanation"))}</td></tr>""")
    return "".join(out)

def role_table(case, role):
    rd = case["pbmc"][role]
    trs = []
    for key in FIELD_ORDER[role]:
        rec = rd.get(key)
        if not rec: continue
        cls = ' class="transaction-main"' if key=="transaction" else ""
        trs.append(f"""<tr{cls}><td>{e(FIELD_NAMES[key])}</td><td>{e(rec.get("value","—"))}</td><td>{e(rec.get("explanation",""))}</td></tr>""")
        if key=="transaction":
            trs.append(flow_rows(case, role))
    actor = rd.get("actor",{})
    return f"""<section class="pbmc-data-block" aria-labelledby="data-{role}">
<div class="data-role-head"><span class="role-dot" style="background:{ROLE_CSS[role]}"></span><span id="data-{role}">{e(ROLE_NAMES[role])}</span><strong>{e(actor.get("value","—"))}</strong></div>
<table class="data-table"><colgroup><col class="field"><col class="value"><col></colgroup>
<thead><tr><th>Field</th><th>Value</th><th>Explanation</th></tr></thead><tbody>{''.join(trs)}</tbody></table></section>"""
...
<section class="section" id="pbmc"><div class="wide-wrap"><div class="section-head"><div><div class="eyebrow">PBMC Snapshot · August 2026</div><h2>Platform Business Model Canvas</h2><p class="section-copy">A visual snapshot of the platform architecture. Hover over a field for its PBMC guiding question and case-specific explanation; hover over an actor or the Core Value Unit for case values.</p></div><div class="canvas-tip">Hover to explore</div></div><div class="canvas-stage"><div class="svg-frame"><svg id="pbmcSvg" viewBox="0 0 1440 960" aria-label="{e(md["company"])} Platform Business Model Canvas"></svg></div><div class="canvas-legend"><span><i style="background:var(--owner)"></i>Owner</span><span><i style="background:var(--provider)"></i>Provider</span><span><i style="background:var(--consumer)"></i>Consumer</span><span><i style="background:var(--partner)"></i>Partner</span></div></div></div></section>
<section class="section soft" id="data"><div class="wrap"><div class="section-head"><div><div class="eyebrow">PBMC Data</div><h2>Structured data behind the canvas</h2><p class="section-copy">The canvas as a reusable reference. Transaction arrows appear as sub-rows of each perspective's Transaction field, so the visual model and the structured data remain one and the same.</p></div><div class="data-actions"><button class="data-btn" id="copyTable" type="button">Copy table</button><a class="data-btn" href="pbmc-data.csv" download>Download CSV</a><a class="data-btn" href="case.json" download>JSON</a></div></div>{''.join(role_table(case,r) for r in ROLES if is_role_enabled(case,r))}<div class="cvu-data"><div><span class="role-name">Core Value Unit</span><strong>{e(case["pbmc"]["core_value_unit"]["value"])}</strong></div><p>{e(case["pbmc"]["core_value_unit"]["explanation"])}</p></div></div></section>
<section class="section" id="video"><div class="wrap"><div class="video-card"><div><div class="eyebrow">Platform Generation · Case Study</div><h3>Watch the story behind the snapshot</h3><p>The video explains why the architecture changed, what changed and why it matters. The library page stays focused on the reusable PBMC itself.</p></div>{video_cta}</div></div></section>
<section class="section soft" id="sources"><div class="wrap"><div class="eyebrow">Evidence</div><div class="section-head"><div><h2>Sources</h2><p class="section-copy">Sources document the platform mechanics and factual case context. The PBMC mapping and Platform Lesson are Platform Generation's analysis.</p></div></div><ol class="sources">{sources_html(case)}</ol></div></section>
<section class="section" id="reuse"><div class="wrap"><div class="reuse-box"><div><div class="eyebrow">Open PBMC</div><h3>Use it. Adapt it. Cite it.</h3><p>This PBMC is released under <strong>{e(reuse["license"])}</strong>. You may reuse and adapt it for research, teaching, workshops and consulting with attribution. Adapted versions should not be presented as an Official PBMC of Platform Generation.</p><p class="citation">{e(reuse["citation"])}</p></div><button class="copy-btn" id="copyCitation" type="button">Copy citation</button></div></div></section>
<section class="case-nav"><div class="wrap case-nav-grid"><a class="case-nav-link" href="../{e(prev["slug"])}/"><span class="case-nav-label">← Previous PBMC</span><span class="case-nav-company">{e(prev["company"])}</span></a><a class="case-nav-link" href="../{e(nxt["slug"])}/"><span class="case-nav-label">{e("Start again" if only_one else "Next PBMC")} →</span><span class="case-nav-company">{e(nxt["company"])}</span></a></div></section>
...
document.getElementById("copyTable").addEventListener("click",async function(){{const cols=["Perspective","Row Type","Field","Value","Direction","Counterparty","Flow","Explanation"];const tsv=[cols.join("\\t"),...tableRows.map(r=>cols.map(c=>String(r[c]??"").replace(/\\t/g," ").replace(/\\n/g," ")).join("\\t"))].join("\\n");try{{await navigator.clipboard.writeText(tsv);this.textContent="Copied";setTimeout(()=>this.textContent="Copy table",1500)}}catch(e){{this.textContent="Use CSV"}}}});
